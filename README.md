...
5. **Lighthouse CI** (PRs only) — runs `@lhci/cli autorun` against `npm run preview` and asserts category budgets (accessibility ≥ 0.9 as a hard gate; performance/best-practices/SEO as advisory warnings). Config in `lighthouserc.json`; full HTML reports upload as the `lighthouse-report` artifact. Complements the static 512 KB JS bundle budget with real rendered-page Core Web Vitals.

...
|-----------|-------------|--------|
| 2.1 App Completeness | No placeholder content, no debug surfaces, no broken links | Done — dev tools removed from production builds (#1425, `check-no-dev-surface.js` in CI, and against the copied native bundle in `cap:build` since LIFT-1454); the bundle cannot be pointed at a dev server (LIFT-1435, `guard:native-config` in `cap:build`); legal links live (#1432) |
...
- [ ] `npm run cap:build` ran green in the shell the archive was cut from — its last two
      steps are what prove the bundle loads its UI from inside the app and not from a LAN
      dev server (`guard:native-config`, LIFT-1435), and that the copied bundle carries
      neither dev surface (`guard:dev-surface --native`, LIFT-1454). Both read the
      generated, gitignored native tree, so no CI run can stand in for them

...
- **Per-gym exercise filtering (#961).** Users training at multiple gyms scope the exercise list to the gym they're at. `Exercise.gyms?: string[]` is multi-membership — **empty/undefined = unassigned = shows under every gym filter** (shared equipment needs no setup); synced via the additive `gyms text[]` column with the always-send `?? []` upsert rule so clears propagate. The gym LIST is a synced preference (`preferences.gyms`, JSONB blob, capped at `MAX_GYMS=10`), while the ACTIVE filter is device-local (`active-gym-filter` localStorage key, deliberately NOT synced — like the overload nudge). Domain logic lives in `src/lib/gyms.ts`; `matchesGymFilter` is the single source of truth, including the **orphan safety net**: membership entries not present in the known gym list are ignored, and a fully-orphaned exercise degrades to visible-everywhere (a rename/delete raced from another device must never hide rows). In WorkoutTracker the gym chip row (reusing `wtTagChip` chrome, no counts, hidden at 0 gyms) is an **exclusive** select applied BEFORE the additive tag OR-filter — tags narrow within the gym, and `tagCounts`/the quick-log picker (`exercisesByRecency`) are gym-scoped to match; drag-reorder is disabled while any filter is active. `effectiveGymFilter` only honors a persisted selection once the hydrated list contains it. CRUD + bulk assignment happen in `GymManagerModal` (a TagManagerModal clone with per-gym exercise checklists), hosted by both WorkoutTracker (manage chip) and SettingsSheet ("Gyms" row — the only zero-state entry point), orchestrated through `useGymActions` (rename rewrites exercise membership; delete strips membership with a restoring undo toast). EditExerciseModal has a Gym multi-select section, rendered only when gyms exist.

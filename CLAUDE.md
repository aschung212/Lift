...
- **Body tab composition.** The "weight" tab renders `BodyView.vue`, a thin wrapper that stacks `BodyweightTracker.vue` (scale weight) and `BodyMeasurements.vue` (chest/arms/waist/thighs circumference). The wrapper keeps each card independently testable and gives the tab a single `KeepAlive` root. Measurements live in the `bodyMeasurements` store (`body-measurements` localStorage key, `body_measurements` Supabase table) and reuse the same sync/tombstone/IndexedDB-backup infra as bodyweight. Values are stored canonically in **centimeters**; the component shows inches for imperial (lbs) users and cm for metric (kg) users.

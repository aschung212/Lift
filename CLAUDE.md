...
- **Shared date helpers live in `src/lib/dates.ts`** (`todayISO`, `toLocalDateKey`, `formatShortDate`, `daysBetweenISO`). The app's mental model is the LOCAL calendar day while the store persists UTC ISO timestamps — never derive a day key via `toISOString().slice(0, 10)`; it rolls evening dates to tomorrow in US timezones (this bug was independently fixed three times before the module existed). `src/lib/storage.ts` provides `loadJSON` for guarded localStorage reads — corrupt storage must never throw into store init.
...
- **Suggestions drawer (#759).** The log-set modal consolidates every "what should my next set be?" affordance — the usual-ladder/last-session quick-fill, the warmup ramp, and the PR-targets table — into ONE segmented disclosure (`.wtSuggestions`, reusing the `wtPrTargets` card chrome). A `currentLens` (`'routine' | 'last' | 'warmup' | 'targets'`) selects the body; `suggestionLenses` lists only those with data (routine OR last, then warmup, then targets). The drawer opens **expanded on the quick-fill lens** (routine when a ladder exists, else last-session) so the one-tap ghost-arm flow is never an extra tap away; warmup + targets sit behind the segmented control. Each lens reuses its prior chip/row markup, so ladder/last-session/warmup/PR-target rendering is unchanged — only the container is unified. The live e1RM estimate stays inline near the inputs (it's feedback that reacts to typing, not a suggestion menu). Segments are 44pt for iOS HIG (regression-tested in `cssRegression.test.ts`).
- **Warmup ramp.** `src/lib/warmupGenerator.ts` exposes `generateWarmupRamp` (pure): scales a working weight by a configurable scheme (default 40/60/80/90% × 8/5/3/1 reps), rounds each step to a loadable plate increment, drops rungs at/above the working weight or that collapse onto a duplicate, and attaches per-side `weightToPlates` breakdowns for barbell loading. The log-set modal renders it as the **Warmup lens** of the Suggestions drawer, whose stable target is the heaviest set of the user's **last session** (read once, not from the live input, so the ramp never reshuffles as warmups are filled/logged). It **coexists** with the usual ladder as a separate segment rather than being suppressed by it (changed in #759): warmup anchors to today's working weight while PR Targets anchor to the all-time PR, so the two can't collapse into one intensity table without breaking adaptive warmups on light days — see #769 for the configurable-intensity-table follow-up. The ramp scheme is **customizable per exercise**: `Exercise.warmupScheme` (`undefined` = default ramp, `[]` = no warmup, custom `{pct,reps}[]` otherwise) is edited in the "Warmup Ramp" section of `EditExerciseModal` (reached via the log-sheet gear button), set through `setExerciseWarmupScheme`, and `sanitizeWarmupScheme` clamps/validates it at every boundary (store setter, localStorage load, remote fetch). It **syncs** via the additive `warmup_scheme jsonb` column (like `bar_weight`) so a custom ramp survives a remote-win merge — the upsert always sends the value (null when unset) so "reset to default" clears it server-side. `Exercise.warmupScheme` (`undefined` = default ramp, `[]` = no warmup, custom `{pct,reps}[]` otherwise) is edited in the "Warmup Ramp" section of `EditExerciseModal` (reached via the log-sheet gear button), set through `setExerciseWarmupScheme`, and `sanitizeWarmupScheme` clamps/validates it at every boundary (store setter, localStorage load, remote fetch). It **syncs** via the additive `warmup_scheme jsonb` column (like `bar_weight`) so a custom ramp survives a remote-win merge — the upsert always sends the value (null when unset) so "reset to default" clears it server-side.

...
- **Boundary validation (LIFT-949).** `src/lib/schema.ts` holds the shared runtime-validation primitives for the deserialization boundary — `isPlainObject`/`isBoolean`/`isNumber`/`isString` guards plus `mergeValidatedOpen` (open `[key: string]: V` maps like `FeatureFlags`, keeps any key whose value validates) and `mergeValidatedKnown` (closed shapes like `ExperienceFlags`/`FilterSettings`, only known keys, unknown keys dropped). **Rule: no `JSON.parse` result may be cast to a domain type without passing through a guard here** (or a domain parser built on one). `preferences.ts` routes its `features`/`experience`/`filters` merges through these so a corrupt cross-tab BroadcastChannel payload can't inject non-boolean flags into reactive state (the old `{ ...DEFAULTS, ...parsed.features }` spread accepted any value type). `jsonColumns.ts` (Supabase-JSON) and the workout/bodyweight localStorage hydration are the remaining boundaries to fold onto this layer.

...
- **Rest-timer Live Activity (#726).** `src/lib/restTimerActivity.ts` mirrors a running rest timer onto the iOS Lock Screen / Dynamic Island via a `registerPlugin('RestTimerLiveActivity')` ActivityKit bridge (same no-op-on-web pattern as `nativeAppIcon.ts`/`appReview.ts`). Gated to native iOS (`isNative && platform === 'ios'`; ActivityKit has no Android/web equivalent) and swallows native failures so the timer flow never breaks. `useRestTimerController` drives the lifecycle: `start` on `startRestTimer`, `update` on pause/resume/restart/duration-change, `end` on stop and on completion. The Swift widget extension lands with the Capacitor iOS build (#531) — until then every call is a no-op. The pure `buildRestTimerActivityState` normalizes the payload (clamps to non-negative ints, caps `remainingSeconds` at `durationSeconds`); `endTimeMs` drives the OS self-updating countdown so the lock-screen timer keeps ticking while JS is suspended. Distinct from the rest-complete notification (#331), which is a one-shot alert.

...
- **Managed timers in composables (#877).** Any composable that owns a `setInterval`/`setTimeout` MUST clear it on scope disposal — a countdown or deferred callback firing after its owning component unmounts mutates torn-down state. Use `useManagedTimers()` (`src/composables/useManagedTimers.ts`): its `setTimeout`/`setInterval`/`clearTimeout`/`clearInterval` wrappers track live ids and auto-clear them via `onScopeDispose` (guarded by `getCurrentScope()`, so it's a safe no-op at module init / in plain unit tests). `useRestTimerController` (the 250ms countdown) and `useXPCeremony` (deferred toast/celebration timeouts) route their timers through it. `usePRBurst` is a module-scope singleton so it can't hold per-instance managed timers — instead it registers a guarded `onScopeDispose` that clears its pending dismiss timeout. Do NOT reach for the bare global `setInterval`/`setTimeout` inside a composable.

...
- **Store reset contract (LIFT-818).** Sign-out (`useAuth.resetStores`) calls `$reset()` on all four stores, and every store's `$reset()` MUST both clear in-memory state AND overwrite its localStorage key(s), then drop `_userId`. The workout store (a setup store) hand-rolls this. The options stores (bodyweight, progression, preferences) **override** Pinia's auto `$reset()` with their own `$reset` action for the same reason: Pinia's default re-runs the state factory, and bodyweight/progression factories call `load()`, which would read the PREVIOUS user's data straight back out of the still-populated localStorage — that stale data then leaks into the next user's Supabase account via `_fetchFromSupabase` on a shared device. Always set `_userId = null` BEFORE calling `_persist()` in a reset (preferences' `_persist` enqueues a Supabase upsert when `_userId` is set). New stores must follow this contract.

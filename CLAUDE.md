...
- **Plate calculator + owned-plate inventory (#835).** `src/lib/plateCalculator.ts` converts between a target weight and a per-side plate stack. `weightToPlates(target, bar, denominations)` now runs a **greedy-first backtracking** search (`solvePerSide`) instead of a one-pass greedy fill: a plain `number[]` of denominations means **unlimited supply** (identical output to the old greedy for canonical sets — backward compatible), while a `PlateStock[]` (`{ denom, count }`, `count` = max appearances per side) restricts the search to plates the user owns, so a limited home-gym set never yields an unrackable suggestion. The owned set is a **global, synced preference** (`preferences.plateInventory`, default `{ enabled: false, lbs: {}, kg: {} }`) stored in the `user_preferences` JSONB blob (no migration), sanitized by `sanitizePlateInventory` (strict-bool `enabled`, known denoms only, pair counts floored to `[0, MAX_PLATE_PAIRS=20]`, zeros dropped). `enabled: false` ⇒ unlimited standard plates (original behavior). Maps store **pairs owned** keyed by denomination string; `ownedPlateStock(inv, unit, perSide)` converts pairs → per-side `count` (doubled in total-loading mode). Edited in Settings → **Plate Inventory** (master toggle that seeds `defaultOwnedPairs` on first enable, then a ±1 stepper per standard denomination for the active weight unit). WorkoutTracker funnels every `weightToPlates`/intensity-table call through one `plateDenominationsArg` computed (falls back to standard plates when inventory is enabled but empty); the manual `addPlate` buttons are capped by `plateSupply` and the button grid hides denominations the user doesn't own. The intensity table's `denominations` option accepts the same union and derives its increment via `denomValues`.

...
- **Workout reminders (#793).** Recurring "time to train" local notifications, native-only. Pure scheduling logic lives in `src/lib/workoutReminders.ts` (`ReminderConfig` = `{ enabled, days: JS-weekday 0–6, hour, minute }`, `sanitizeReminderConfig`, `buildReminderNotifications`) — fully tested without Capacitor. The native bridge `src/lib/localNotifications.ts` wraps `@capacitor/local-notifications` via a runtime-constructed dynamic import (same `/* @vite-ignore */` pattern as `appReview.ts`, so the web bundle never pulls it in) and is a no-op on web (scheduled local notifications don't fire in a PWA). `useWorkoutReminders.syncReminders` reconciles the OS schedule by **always cancelling the seven deterministic slot IDs** (`REMINDER_ID_BASE + jsDay`) first, then scheduling the active set — idempotent re-sync, never duplicates. Config is a **synced preference** (`preferences.workoutReminders`, default off / Mon-Wed-Fri 6pm) in the `user_preferences` JSONB blob (no migration). Settings → **Reminders** section is gated behind `isNative` (like the app-icon picker). Capacitor weekday is 1-based (Sun=1), so `toCapacitorWeekday` shifts at the bridge boundary only.

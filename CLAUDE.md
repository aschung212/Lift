...
- **Warmup ramp.** `src/lib/warmupGenerator.ts` exposes `generateWarmupRamp` (pure): scales a working weight by a configurable scheme (default 40/60/80/90% × 8/5/3/1 reps), rounds each step to a loadable plate increment, drops rungs at/above the working weight or that collapse onto a duplicate, and attaches per-side `weightToPlates` breakdowns for barbell loading. The log-set modal renders it as a collapsible card (reusing the `wtPrTargets` card styling, collapsed by default — progressive disclosure) whose stable target is the heaviest set of the user's **last session** (read once, not from the live input, so the ramp never reshuffles as warmups are filled/logged). Deliberately suppressed when the usual ladder is active — that row already encodes habitual warmups, so showing both would be a parallel path.

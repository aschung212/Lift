...
# e2e + migrate-db (~6 minutes, e2e alone 3-4), the production alias (read from
# CLAUDE.md by the workflow, deliberately not named here) only ever serves the
# LATEST ready deployment, and master runs serialize behind `concurrency`.
# So merging two PRs 90 seconds apart leaves the first one's job

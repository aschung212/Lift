 ...
 * verify the artifact.
 *
 * It deliberately does NOT run in CI, unlike its `guard:dev-surface` sibling: the
 * file it reads is generated by `cap sync` and gitignored, so a CI checkout never
 * has one and every run would fail "never been synced". The half that CAN be
 * checked without a native toolchain — that `capacitor.config.ts` resolves to no
 * dev-server origin under `CAPACITOR_BUILD=true` — is unit-tested instead
 * (`capacitorConfigRegression.test.ts`). This script is the local archive-time
 * gate for the artifact itself.
 *
 * Usage:

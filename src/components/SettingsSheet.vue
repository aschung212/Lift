    ...
    <div v-if="modelValue" class="settingsOverlay" @click.self="closeSettings" @keydown.escape="closeSettings">
      <div class="settingsSheet" :ref="onSettingsSheetMounted" :style="settingsSwipe.dragStyle()" role="dialog" aria-modal="true" aria-labelledby="settings-title">
        ...
        <div class="settingsGroup">
          ...
          <!-- Tip jar (native IAP only — App-Store-compliant, LIFT-910) -->
          <template v-if="tipJar.available">
            <p class="settingsHint settingsTipIntro">Lift is free with no ads. If it helps your training, you can leave an optional tip.</p>
            <button
              v-for="tier in tipJar.tiers"
              :key="tier.id"
              class="settingsRow settingsRowBtn"
              :disabled="tipInFlight"
              @click="sendTip(tier.id)"
            >
              <span class="settingsLabel">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" width="16" height="16" style="vertical-align: -2px; margin-right: 6px; color: var(--accent)"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 1 0-7.78 7.78L12 21.23l8.84-8.84a5.5 5.5 0 0 0 0-7.78z"/></svg>
                {{ tier.label }}
              </span>
              <span class="settingsHint">{{ tipPrices[tier.productId] || '' }}</span>
            </button>
            <div v-if="tipFeedback" class="settingsImportResult" role="status">
              <span class="settingsImportSuccess">{{ tipFeedback }}</span>
            </div>
          </template>
...
import { getTipPrices, type TipTierId } from '../lib/tipJar'
...
// ── Tip jar (native IAP only, LIFT-910) ────────────────────────
// App-Store-compliant "Support Lift" purchase — a one-time StoreKit consumable.
// Only exists on the native build (Apple forbids external purchase links there,
// guideline 3.1.1); on web this whole block is hidden and the external Sponsor /
// Buy-Me-a-Coffee links remain.
const tipJar = useTipJar()
const tipPrices = ref<Record<string, string>>({})
const tipFeedback = ref<string | null>(null)
let tipFeedbackTimer: ReturnType<typeof setTimeout> | null = null
const tipInFlight = computed(() => tipJar.status.value === 'purchasing')

if (tipJar.available) {
  tipJar.logImpression()
  void getTipPrices().then((prices) => { tipPrices.value = prices })
}

async function sendTip(tierId: TipTierId) {
  if (tipInFlight.value) return
  tipFeedback.value = null
  const status = await tipJar.tip(tierId)
  if (status === 'completed') tipFeedback.value = 'Thank you for supporting Lift!'
  else if (status === 'error' || status === 'unavailable') tipFeedback.value = 'Purchase unavailable — try again later'
  // A user cancellation is silent (no feedback line).
  if (tipFeedback.value) {
    if (tipFeedbackTimer) clearTimeout(tipFeedbackTimer)
    tipFeedbackTimer = setTimeout(() => { tipFeedback.value = null; tipJar.reset() }, 3000)
  }
}

<template>
  ...
  <Teleport to="body">
    <div class="settingsOverlay" @click.self="close" @keydown.escape="close">
      <div class="settingsSheet" :ref="onSheetMounted" :style="swipe.dragStyle()" role="dialog" aria-modal="true" aria-labelledby="settings-title">
...
// ── Composables & stores (shared module-level singletons) ─────
...
function onSheetMounted(el: Element | ComponentPublicInstance | null) {
  ...
  if (el instanceof HTMLElement && el !== sheetEl.value) {
    ...
    nextTick(() => {
      ...
      if (handle) swipe.attach(el, handle)
    })
...
// Cleanup when unmounted (e.g. force-closed from parent)
watch(() => sheetEl.value, (el, oldEl) => {
  if (!el && oldEl) {
    swipe.detach()

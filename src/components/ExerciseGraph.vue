...
const {
  ...
  gridYs,
  shouldShowLabel,
} = useSVGTimeSeries(timeSeriesEntries, minVal, maxVal)

// Augment mapped points with PR flag
const points = computed(() => {
  const prDate = graphData.value.find(([, v]) => v === maxVal.value)?.[0] ?? ''
  return basePoints.value.map(p => ({
    ...p,
    e1rm: p.value,
    isPR: p.date === prDate,
  }))

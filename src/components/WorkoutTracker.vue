<template>
  ...
  <!-- Rest timer modal (self-contained) -->
  <RestTimerView
    v-if="showRestTimerModal"
    :exercise-name="selectedExerciseName"
    @skip-to-next="skipToNextSet"
    @dismiss="dismissTimer"
    @close="showRestTimerModal = false"
    @restore="showRestTimerModal = true"
  />

  <!-- Log / Edit Set Modal -->
  <Teleport to="body">
    <div v-if="showModal" class="repMaxOverlay logSetOverlay" @click.self="onOverlayClick" @keydown.escape="closeModal">
      <div ref="logSheetEl" class="repMaxModal logSetSheet" :style="logSwipe.dragStyle()" role="dialog" aria-modal="true" aria-labelledby="log-modal-title">
  ...
  <!-- Rest timer bar -->
  <RestTimerBar :show-modal="showModal || showRestTimerModal" @open-timer="openRestTimer" />
...
import RestTimerView from './RestTimerView.vue'
import RestTimerBar from './RestTimerBar.vue'
...
const showRestTimerModal = ref(false)
...
function closeModal() {
  ...
  restTimer.editingPresets.value = false
  editingSet.value = null
  selectedExerciseId.value = ''
...
// ── Rest timer (delegated to useRestTimerController) ────────────
// Keep screen awake while the rest timer is running or the log-set modal is open
const wakeLockNeeded = computed(() => restTimer.timerActive.value || showModal.value || showRestTimerModal.value)
useWakeLock(wakeLockNeeded, wakeLockEnabled)

// Wire timer-complete callback to skip to next set
restTimer.setOnTimerComplete(() => skipToNextSet())

function skipToNextSet() {
  restTimer.stopTimer()
  showRestTimerModal.value = false
  date.value = lastLogDate.value
}
...
function openRestTimer() {
  showRestTimerModal.value = true
  if (!restTimer.timerActive.value) {
...
function saveSet() {
  ...
  if (isEditMode.value && editingSet.value && weight.value !== null && reps.value !== null) {
    ...
    if (hasSetData.value && weight.value !== null && reps.value !== null) {
      ...
      if (restTimer.restTimerEnabled.value && restTimer.restTimerAutoStart.value) {
        restTimer.startTimer()
        showRestTimerModal.value = true
      }
...
watch(
  () => showModal.value || showRestTimerModal.value || !!detailExerciseId.value || editTarget.value !== null || tagManagerOpen.value,

...
import { sanitizeGymName, MAX_GYMS, GYM_NAME_MAX_LENGTH } from '../lib/gyms'
...
// ── Gym / search / tag filtering (LIFT-915) ──────────────────────
// The whole filter stack — its state, its device-local persistence, and every
// list derived from it — lives in `useExerciseFilters`. The three branch
// points matter here: `exercisesByRecency` feeds the quick-log picker (gym
// only), `planScopeExercises` feeds the session-plan card (gym + tags, store
// order), and `filteredExercises` feeds the main list (everything).
const {
  searchQuery,
  activeTagFilters,
  activeGymFilter,

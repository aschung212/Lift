...
export const usePreferencesStore = defineStore('preferences', {
  state: () => ({
    ...
    /** Recurring local "time to train" reminder schedule (#793, native only). */
    workoutReminders: { ...DEFAULT_REMINDER_CONFIG } as ReminderConfig,
  ...
  actions: {
    _persist() {
      const payload = {
        ...
        workoutReminders: this.workoutReminders,
      }
    ...
    _reloadFromStorage() {
      ...
      try {
        ...
        if (parsed.workoutReminders) this.workoutReminders = sanitizeReminderConfig(parsed.workoutReminders)
      ...
      if (supabase) {
        try {
          ...
          if (prefs?.features) {
            ...
            if (prefs.workoutReminders) this.workoutReminders = sanitizeReminderConfig(prefs.workoutReminders)
    ...
    /** Replace the recurring workout-reminder config (sanitized: valid days/time). */
    setWorkoutReminders(config: ReminderConfig) {
      this.workoutReminders = sanitizeReminderConfig(config)
      this._persist()
    },
  },

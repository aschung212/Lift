...
export const useWorkoutStore = defineStore('workout', () => {
  // ── State ──────────────────────────────────────────────────────
  // shallowRef avoids wrapping every nested Exercise/WorkoutSet in a Proxy.
  // For a user with 100 exercises × 50 sets = 5,000 set objects, this
  // eliminates ~5,000+ Proxy wrappers. Mutations must call triggerRef()
  // (centralized in _persist) to notify Vue of changes.
  const exercises = shallowRef<Exercise[]>(load())
  const customTags = ref<string[]>(JSON.parse(localStorage.getItem('lift-custom-tags') || '[]'))
  const tagRecoveryDays = ref<Record<string, number>>(JSON.parse(localStorage.getItem('lift-tag-recovery-days') || '{}'))
  const tagRecoveryExcluded = ref<string[]>(JSON.parse(localStorage.getItem('lift-tag-recovery-excluded') || '[]'))
  ...
  function _persist() {
    ...
    // Notify Vue that the shallow exercises ref has been mutated
  ...
  function _adoptExercise(exercise: Exercise) {
    ...
    if (supabase && !isPreviewMode.value && _userId.value) {
  ...
  async function _fetchFromSupabase() {
    if (!supabase || !_userId.value) return

    const [{ data: fetchedExercises }, { data: sets }] = await Promise.all([
      supabase.from('exercises').select('*').eq('user_id', _userId.value).is('deleted_at', null).order('created_at'),
      supabase.from('sets').select('*').eq('user_id', _userId.value).is('deleted_at', null).order('created_at')
    ])

    if (!fetchedExercises) return

    // Filter out tombstoned exercises (deleted offline, not yet synced)
    const remoteIds = new Set(fetchedExercises.map((ex: Record<string, unknown>) => ex.id as string))
    ...
    // Reassigning .value auto-triggers shallowRef reactivity
    ...
    const tombstoneExercises = fetchedExercises
  ...
  function addExercise(name: string, tags: string[] = [], { sync = true }: { sync?: boolean } = {}): string | null {
    ...
    if (sync && supabase && !isPreviewMode.value && _userId.value) {
      const userId = _userId.value
  ...
  function renameTag(oldName: string, newName: string) {
    ...
    const customIdx = customTags.value.indexOf(oldName)
    if (customIdx !== -1) {
      if (customTags.value.includes(trimmed)) {
        customTags.value.splice(customIdx, 1)
      } else {
        customTags.value[customIdx] = trimmed
      }
    }
    if (oldName in tagRecoveryDays.value) {
      const days = tagRecoveryDays.value[oldName]
      delete tagRecoveryDays.value[oldName]
      if (!(trimmed in tagRecoveryDays.value)) {
        tagRecoveryDays.value[trimmed] = days
      }
    }
    const exclIdx = tagRecoveryExcluded.value.indexOf(oldName)
    if (exclIdx !== -1) {
      if (!tagRecoveryExcluded.value.includes(trimmed)) {
        tagRecoveryExcluded.value[exclIdx] = trimmed
      } else {
        tagRecoveryExcluded.value.splice(exclIdx, 1)
      }
    }
    _persist()

    if (_userId.value && modified.length > 0) {
      const userId = _userId.value
  ...
  function deleteTag(tagName: string) {
    ...
    delete tagRecoveryDays.value[tagName]
  ...
  function setTagRecoveryDays(tag: string, days: number | null) {
    if (days === null || days <= 0) {
      delete tagRecoveryDays.value[tag]
    } else {
      tagRecoveryDays.value[tag] = days
    }
    _persist()
  }

  function setTagRecoveryExcluded(tag: string, excluded: boolean) {
    const idx = tagRecoveryExcluded.value.indexOf(tag)
    if (excluded && idx === -1) {
      tagRecoveryExcluded.value.push(tag)
    } else if (!excluded && idx !== -1) {
      tagRecoveryExcluded.value.splice(idx, 1)
    }
    _persist()
  }

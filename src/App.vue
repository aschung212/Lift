    ...
    <template v-else>
      <main class="appContainer">
        ...
        <Transition name="installBanner">
          <div v-if="showInstallBanner" class="installBanner" role="banner">
            <div class="installBannerContent">
              <div class="installBannerIcon" aria-hidden="true">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
              </div>
              <div class="installBannerText">
                <template v-if="isIOSPrompt">
                  Tap <svg class="installBannerInlineIcon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-label="Share"><path d="M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8"/><polyline points="16 6 12 2 8 6"/><line x1="12" y1="2" x2="12" y2="15"/></svg> then <strong>Add to Home Screen</strong> for the full app experience.
                </template>
                <template v-else>
                  Install Lift for quick access from your home screen.
                </template>
              </div>
            </div>
            <div class="installBannerActions">
              ...
              <button class="installBannerBtn installBannerDismiss" @click="dismissInstall" aria-label="Dismiss install prompt">Not now</button>
            </div>
          </div>
        </Transition>
        <div class="appTopBar">
...
// ── PWA install prompt ─────────────────────────────────────────
const { showBanner: showInstallBanner, isIOSPrompt, install: triggerInstall, dismiss: dismissInstall, check: checkInstall } = useInstallPrompt(
  () => workoutStoreForOnboarding.workoutDates.length,
)
// Re-check eligibility when workout dates change (new sessions logged)
watch(() => workoutStoreForOnboarding.workoutDates.length, () => checkInstall())

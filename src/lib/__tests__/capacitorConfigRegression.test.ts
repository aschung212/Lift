...
// LIFT-1435: the config is what decides where the WebView loads the app FROM,
// so it is asserted by evaluating it, not by reading it. `cap sync` resolves
// this module and writes the answer into ios/App/App/capacitor.config.json —
// gitignored, copied into the .ipa, and read by the iOS runtime — so a
// CAPACITOR_DEV_URL that survives into a release build ships an App Store app
// whose entire UI comes over plaintext HTTP from a LAN address. Both directions
// matter: the fix must not simply delete live reload.

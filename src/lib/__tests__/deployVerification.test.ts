...
describe('production deploy verification (LIFT-1167)', () => {
  ...
  describe('the "Deployed" claim is withheld when nothing deployed', () => {
    ...
    it('notify-deploy reads the gate decision and branches its message', () => {
      ...
      // this rule over a branch it was never about.
...
// master push lands inside the job's own runtime. It does not: the job waits
// on build-and-test + e2e + migrate-db (~6 minutes), and merging a backlog
// puts pushes 90 seconds apart. Three PRs merged in a row on 2026-09-13, all
// three deployed, and the middle one polled 300s for a SHA the alias had
// already moved past — then fired 🔴 for a deploy that had succeeded.
//
...
describe('the workflow consumes the classifier (LIFT-1414)', () => {
  ...
  it('checks out enough history for ancestry to be decidable', () => {
    // In a shallow clone git treats the commits on the graft boundary as
    // parentless, so `merge-base --is-ancestor` can answer "no" for a
    // descendant that really is one — re-creating this issue's false alarm
    // from the other side.
  ...
  it('names which failure it saw instead of blaming the deploy for all of them', () => {
    ...
    // must not name the wrong one. "The Vercel build failed or stalled" is
    // true of a production that is BEHIND and a misattribution for every other
    // way the window can run out — which is how a markdown edit (LIFT-1412)
    // and a superseded alias (this issue) both came to be reported as outages.
    ...
    // And only the one that really means "the deploy did not land" says so.
    const blamesTheBuild = [...arms.entries()].filter(([, msg]) =>
      /(build|deploy)[^.]*(fail|stall)/i.test(msg),
    )
    expect(blamesTheBuild.map(([token]) => token)).toEqual([BEHIND])
  })
  ...
  it('publishes a superseded verification so the Slack claim stays exact', () => {
    ...
    const branch = notifyRun.slice(notifyRun.indexOf(`-n "$${envEntry?.[0]}"`))
    expect(branch, 'the message must branch on it').not.toBe('')
    ...
    const message = branch.slice(0, branch.indexOf('\n  else'))

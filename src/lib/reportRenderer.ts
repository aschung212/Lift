...
<title>Logbook Training Report — ${escapeHtml(report.periodLabel)}</title>
<style>
...
<body>
  <nav class="report-toolbar" aria-label="Report navigation">
    <button type="button" class="back-button" id="back-to-logbook" aria-label="Close report and return to Logbook">
  ...
  <div class="print-hint">
    Press <strong>Cmd+P</strong> (Mac) or <strong>Ctrl+P</strong> (Windows) to save as PDF, then tap <strong>Back to Logbook</strong> to return
  </div>
  ...
  <div class="footer">
    Generated by Logbook — ${new Date().toISOString().slice(0, 10)}
  </div>

  <script>
    (function () {
      var btn = document.getElementById('back-to-logbook');
      if (!btn) return;
      btn.addEventListener('click', function () {
        ...
        // reload Logbook inside a tab the browser is already tearing down.
